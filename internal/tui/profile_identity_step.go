...
func (m Model) renderProfileIdentityPanelContent(w, h int, t homeTheme) wizardPanel {
	pw := min(max(w-12, 34), profileStepPanelMaxWidth)
	if pw > w {
		pw = w
	}
	s, inset := profileStepPanelStyle(t, h)
	cw := max(pw-inset, 1)
	...
	if m.status != "" {
		...
		lines = append(lines, m.renderFeedback(cw, t))
	}
	lines = appendWizardGap(lines, rhythm.actions)
	actions := renderWizardActionsBlock(cw, t, "< VOLVER >", "[ CONTINUAR ]", m.identityFocus == profileIdentityFocusBack, m.identityFocus == profileIdentityFocusContinue, m.noColor, wizardActionOptions{rightEnabled: m.identityDecision != profileIdentityNone})
	...
	// The shared panel always contributes its top border; the generous layout
	// additionally contributes one row of vertical padding before content.
	offset := 1
	if h >= 30 {
		offset++
	}
	for focus, r := range ranges {
		ranges[focus] = wizardLineRange{start: r.start + offset, end: r.end + offset}
	}
	return wizardPanel{text: centerHomeBlock(w, s.Width(pw-2).Render(strings.Join(lines, "\n"))), ranges: ranges}
}
