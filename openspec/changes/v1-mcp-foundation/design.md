...
For tasks regeneration, split the former PR 5 credential/security/audit unit into ≤400-line stacked-to-main slices: **5A** first performs the unchanged fail-closed keyring PoC-exception dependency verification, then adds the consumer-owned credential contract, keyring adapter, and consumer/per-OS tests; **5B** adds client authorization, pinned TOFU, sanitized audit, and their tests. PR 3B.1 starts with the modernc v1.54.0 exact-graph/license/security/platform admission above before SQLite implementation. PRs 3B.1–3B.3 and all later non-PR-5 ordering remain unchanged.
...
None for PoC planning: the modernc v1.54.0 baseline and keyring v0.2.8 exception are approved, and PR 3B.1/5A verification failures stop delivery rather than reopen product decisions. Production/corporate dependency and endpoint-policy approval remains unresolved and mandatory as a rollout gate. No unsupported portable filesystem or power-loss guarantee is claimed.
