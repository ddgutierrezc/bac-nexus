...
| 2.18 pre-acquire recovery gate | `internal/source/acquire_test.go`; source fake unit layer | `go test -count=1 ./internal/source` passed before the declaration/test change. | Commit `36e59f4` added the behavior tests plus only the minimal `Recover` function-field seam; the focused command failed because missing, failed, and cancelled recovery still opened/acquired, and successful recovery was not called. | The minimum gate validates context/dependencies, invokes recovery, then opens acquisition; focused source tests passed after `200166e`. | Missing dependency, recovery error, cancelled context, and success cover fail-closed/no-open/no-copy/no-admit and one successful existing acquisition path. | The named `successfulAcquisitionRecovery` no-op is test-only; production composition must supply recovery explicitly. |
| 2.19 security documentation | `docs/SECURITY.md`; documentation layer | Existing source-package safety net passed before documentation. | N/A — documentation-only refactor with no production behavior. | N/A — no documentation test runner is configured. | The document separately covers exact ownership cleanup, pre-acquire blocking, privileged/same-principal risk, evidence limits, rollout gates, and rollback. | No fixture refactor was needed beyond the explicit test-only recovery fixture required by task 2.18. |
...
| Evidence | Result |
|---|---|
| Focused test command and exact result | `go test -count=1 ./internal/source -run 'TestAcquirer(RequiresSuccessfulRecoveryBeforeOpeningRemote|RecoversExactlyOnceBeforeExistingAcquisition)$'` failed as RED on commit `36e59f4`; after the minimum gate, `go test -count=1 ./internal/source` exited 0. Final repository-wide verification is carried by the delivery result rather than this progress artifact. |
| Runtime harness command/scenario and exact result | N/A locally: this slice has no live IBM i or process-startup boundary. Package-local fakes prove recovery failure/cancellation prevents acquisition open/copy/admit, and success retains the existing flow. GitHub Actions is the available CI runtime harness; unavailable platform/runtime claims are not inferred. |
