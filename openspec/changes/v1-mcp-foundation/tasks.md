...
- [ ] 2.17 **Recovery (3B.3)**: Slice C implementation awaits final artifact-head GREEN verification; Slice D owns later startup/pre-acquire invocation.
...
Only canonical parent tasks use checkboxes: native count is 42, with 25 complete through 2.16. Tasks 2.17–2.19 remain pending until Slice C final artifact-head verification is green; Slice D follows that settlement. Every Slice B–D RED must compile against its preceding GREEN and prove independent behavior. Run `go test -count=1 ./internal/source ./internal/ownership/sqlite` in GitHub Actions for every later pair; WDAC blocks local test runtime and must not be bypassed. Roll back in reverse dependency order: Slice D/2.19, Slice C, Slice B, then Slice A; retain unresolved rows and never introduce an MCP recovery operation.
