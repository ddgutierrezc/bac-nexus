...
- [ ] 2.4 **GATE (PR 3B.1)**: Verify Go 1.25 minimum (evaluated Go 1.27.0 windows/amd64); PoC-exception `modernc.org/sqlite` v1.54.0 (`h1:qNQP6xnx5M0ISNtlnxoOX0+cD5bJ0/gr9aMmndFczzg=`; SQLite 3.53.3; pure-Go/no-CGO; windows/darwin/linux amd64+arm64); explicitly resolve `golang.org/x/mod` v0.40.0+ fixed Go-1.25-compatible (evaluated `x/mod` v0.40.0, `x/sys` v0.47.0); inventory full graph/SBOM/checksums/licenses, separating runtime/shipped from tooling/test-only without omitting either; runtime admission: direct OSV + `govulncheck`, no unresolved runtime vulnerability, DLL/runtime download, service/listener, or admin dependency; six CGO-disabled compile targets plus consumer open/schema/roundtrip/pragmas tests on available runners only—never upstream recursive race/CGO suite; endpoint policy; production rollout approval remains unresolved and mandatory. Failure blocks 3B.1.
