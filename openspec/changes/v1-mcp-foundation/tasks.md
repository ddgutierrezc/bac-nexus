...
Only canonical parent tasks use checkboxes: native count is 42, with 26 complete through 2.17. Tasks 2.18 and 2.19 remain pending; Slice D is next. Every Slice B–D RED must compile against its preceding GREEN and prove independent behavior. Run `go test -count=1 ./internal/source ./internal/ownership/sqlite` in GitHub Actions for every later pair; WDAC blocks local test runtime and must not be bypassed. Roll back in reverse dependency order: Slice D/2.19, Slice C, Slice B, then Slice A; retain unresolved rows and never introduce an MCP recovery operation.
