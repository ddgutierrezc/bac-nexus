...
- [x] 7.3.4 `internal/tui/model.go`: inject only `configuration.Step8Runner`; assert Steps 3–4 pre-auth/zero runner-credential-runtime calls; retain credential-free runtime-free state.
...
Exact checkbox totals: **35 total = 28 completed + 7 pending**. Tasks through 7.3.4 source/tests and hybrid task/progress evidence are complete; no native/VCS/.atl/tmp edits occurred.
