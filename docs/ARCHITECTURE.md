...
- `internal/connectors/ibmi/mapepirestdio` owns the current IBM i SSH/stdio launch policy: the pinned 2.3.6 JAR receipt, discovery and verification, artifact activation/rollback, Java safety rules, environment, and `--single` command shape.

Composition selects the current Mapepire session as the Catalogados executor. A future official `mapepire-go` adapter can replace that executor/session at composition without changing `internal/catalog`, `internal/app`, or the MCP contract. Live composition is exercised only by the external controlled gate and remains `not_validated_on_ibmi`.
