...
This document records the **Create IBM i profile** wizard without claiming live IBM i validation, profile migration, or `nexus serve` readiness. Current code, tests, and active OpenSpec change `complete-ibmi-profile-wizard` outrank the historical detail retained below.

## Executive status

**FACT:** the canonical flow has eight steps: Profile, Connection, Server Identity, Mapepire Readiness, Credentials, Review & Save, Optional Remote Proof, and Completion. No phantom step is shown. Step 3 preserves explicit TOFU; a rotated pinned identity fails closed. Step 4 remains a bounded pre-auth readiness observation. Steps 5–8 preserve secret opacity, explicit consent, and truthful local-only completion outcomes.
...
**NOTE:** this document describes only the canonical eight-step wizard. It must not be used to infer remote proof, IBM i, or `nexus serve` readiness.
...
**Scope:** the local `nexus configure` wizard and the lower-layer capabilities it may compose later. It does not redefine the MCP server, authorize IBM i access, or claim field validation.
...
When sources conflict, use this order: current code and tests; valid OpenSpec; current docs; archived OpenSpec; this explanatory document. Historical verification proves only the stated configuration slice.

## Global architecture and journey contract

### What a profile means

**FACT:** a persisted `profile.Profile` is non-secret metadata: name, host, SSH port, username, pinned host-key fingerprint and provenance, optional managed-runtime settings, optional local Mapepire JAR path, and credential mode. Profile JSON does not contain secret material.
...
flowchart LR
  ...
  S4 --> S5[5 Credentials]
  S5 --> S6[6 Review & Save]
  S6 -. optional remote proof .-> S7[7 Optional Remote Proof]
  S7 --> S8[8 Completion]
  S1 <-- Back --> H
  S2 <-- Back --> S1
  S3 <-- Back --> S2
...
## Canonical eight-step journey

The ordered lifecycle is **Step 1 Profile; Step 2 Connection; Step 3 Server Identity; Step 4 Mapepire Readiness; Step 5 Credentials; Step 6 Review & Save; Step 7 Optional Remote Proof; Step 8 Completion.** Step 4 is limited to pre-auth readiness; later steps retain ownership of authentication and proof.
...
| Current UX/TUI | **FACT:** `Crear perfil IBM i`, `Paso 1 de 8 — Perfil`, real Bubbles input and block cursor, local loading/duplicate feedback, `< CANCELAR >`, and `[ CONTINUAR ]`. |
...
| Current UX/TUI | **FACT:** `Paso 2 de 8 — Conexión`, real inputs, visible block cursor, `< VOLVER >`, `[ CONTINUAR ]`, and copy that Nexus will not yet connect. |
...
| Current UX/TUI | **FACT:** one `Paso 3 de 8 — Identidad` has authorization, loading/error, observed-identity review, and completed in-memory substates. It displays the draft host/port, then complete observed algorithm/fingerprint, and accepts through `[ CONFIAR EN ESTA CLAVE ]`. Review Back returns to authorization and clears unaccepted evidence; completed Back returns to Step 2 while retaining accepted evidence for the unchanged endpoint. |
...
| Decisions / proposal / missing work | **DECISION:** observed is not independently verified and changed pinned keys fail closed. **FUTURE:** profile persistence remains the Review/Save boundary; later canonical steps remain governed by their approved specifications. |
| Status | **FACT:** Partial — explicit TOFU inspection and in-memory preparation exist; profile persistence and later steps do not. |

#### Independently verified fingerprint option
...
| Current UX/TUI | **FACT:** uses the shared wizard shell, panel, actions, feedback, viewport, responsive sizes, and `NO_COLOR`; it preserves `Step 4 of 8 — Mapepire`. |
...
| Continue does not do | It does not look up credentials, authenticate, select/fallback transports, invoke SSH/JAR/upload/cache, or query IBM i. |
...
**Current OpenSpec contract:** Mapepire readiness is pre-auth only. JAR, artifact, upload, and launch work remains an authenticated SSH fallback concern and is not implied by daemon readiness.
...
| Requested information / actions | Name, endpoint, user, port, trust provenance/fingerprint status, local Mapepire selection, and credential presence/mode—never a secret. Back corrects any earlier draft. |
...
| Continue does | **PROPOSAL:** after explicit confirmation, atomically save the secret-free profile metadata exactly here, then enter Step 7 when optional proof is selected or Step 8 when it is omitted. |
| Continue does not do | **PROPOSAL:** saving must not imply IBM i connectivity, successful authentication, Mapepire launch, or live validation. |
...
| Requested information / actions | Consent, visible scope, timeout, cancellation, sanitized outcome, and a choice to skip. Candidate checks may include pinned-host verification, authentication, Mapepire artifact/launch check, and a read-only service probe. |
| Current UX/TUI | **FACT:** an injected Step 7 action screen renders bounded running, success, terminal, and cancelled feedback. It rejects stale request IDs, retry creates a new request, and `View()` performs no I/O. This lifecycle requires the saved V3 profile from Review & Save and explicit fallback consent. |
...
| Status | **FACT:** Partial — service and action lifecycle contracts exist, but live field validation does not. |

#### Step 8 evidence and live-validation boundary

The current evidence is intentionally offline. It proves deterministic contract behavior; it does **not** prove production network, IBM i, SSH, artifact transfer, upload/download, keyring, credential, or bank-environment behavior.
...
| Current UX/TUI | **FACT:** Completion distinguishes omitted, cancelled, failed, and successful proof outcomes without claiming live validation. |
...
## Evaluation of the eight-step UX

The eight-step sequence makes valuable boundaries visible: profile identity, endpoint syntax, server trust, pre-auth readiness, credentials, review/save, optional remote proof, and completion.
...
| Keep Mapepire readiness limited to its pre-auth boundary. | **DECISION** |
| Make Step 8 visibly optional and preserve the Step 7 save-versus-test distinction. | **PROPOSAL** |
| Keep Step 4 limited to pre-auth readiness; runtime activation remains outside the wizard. | **DECISION** |
...
| 5 Credentials | Complete | Complete | Partial | Complete | Partial |
| 6 Review & Save | Complete | Complete | Partial | Complete | Partial |
| 7 Optional Remote Proof | Complete | Complete | Partial | Complete | Partial |
| 8 Completion | Complete | Complete | Partial | Complete | Partial |
...
| Managed runtime launch policy | Outside wizard | Does not discover or verify a remote runtime. |
...
| Managed runtime validation (outside wizard) | No | Yes | Yes | No, unless explicit status persistence is approved |
...
3. Preserve Steps 5–8 credential, review/save, proof, and completion boundaries with secret-safe UX and one atomic saved-profile boundary.
4. Keep Step 7 reachable only from a newly saved profile and preserve explicit fallback consent and save-versus-proof separation.
5. Keep Step 8 completion local and truthful without claiming live IBM i or `nexus serve` readiness.
...
| Mapepire scope | Step 4 is pre-auth readiness only. | Keep runtime activation outside the wizard. |
...
- **PROPOSAL:** local Mapepire discovery UI if the OpenSpec scope changes.

### DEFERRED

- **FUTURE:** automatic remote runtime discovery/verification during entry.
...
3. **Mapepire wizard scope:** whether the valid legacy-diagnostics classification changes beyond Step 4 readiness.
...
| [Current configuration spec](../openspec/specs/nexus-configuration/spec.md) / [local MCP security spec](../openspec/specs/local-mcp-security/spec.md) / [IBM i catalog-context spec](../openspec/specs/ibmi-catalog-context/spec.md) | Valid requirements for configuration, security/credential/trust/diagnostics, and bounded MCP/catalog behavior. |
