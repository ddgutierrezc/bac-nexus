...
describe("fixed Windows descriptor token store", () => {
  ...
  it("keeps the two inbox scripts limited to protected descriptor publication and owned cleanup", async () => {
    ...
    expect(publish).toMatch(/\$directoryAccessRule = \[System\.Security\.AccessControl\.FileSystemAccessRule\]::new\([\s\S]*?\r?\n\s*\)\r?\n\s*\[Console\]::Out\.Write\("ACCESS_RULE_CREATED`n"\)\r?\n\s*\[Console\]::Out\.Flush\(\)/);
